...
validate.extend(validate.validators, {requiredWithAll});
...
describe('requiredWithAll', function() {
  ...
  test('single attribute, condition true', function() {
    ...
    expect(result.value).toHaveLength(1);
    expect(result.value[0]).toEqual('Value required when all of these attributes (other) are present');
  });
  ...
  test('multi attribute, all filled, condition true', function() {
    ...
    expect(result.value).toHaveLength(1);
    expect(result.value[0]).toEqual('Value required when all of these attributes (other, other2) are present');
  });
